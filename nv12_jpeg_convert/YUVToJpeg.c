...
)
{
	...
	cinfo.err = jpeg_std_error(&jerr);  // Errors get written to stderr 
	jpeg_create_compress(&cinfo);
	cinfo.image_width = nNV12Width;
	cinfo.image_height = nNV12Height;
	cinfo.input_components = 3;
	jpeg_set_defaults(&cinfo);
	jpeg_set_colorspace(&cinfo, JCS_YCbCr);
	cinfo.raw_data_in = TRUE; // Supply downsampled data
	...
	int fStrides[2] = { nNV12Width, nNV12Width };
	unsigned char* yPlanar = (unsigned char*)pDataNV12;
	unsigned char* vuPlanar = (unsigned char*)pDataNV12 + nNV12Width * nNV12Height; //width * height;
	unsigned char* uRows = (unsigned char*)malloc(8 * (nNV12Width >> 1));
	unsigned char* vRows = (unsigned char*)malloc(8 * (nNV12Width >> 1));
	...
	while (cinfo.next_scanline < cinfo.image_height)
	{
		...
		{
			//deitnerleave u and v
			deinterleave(vuPlanar, uRows, vRows, cinfo.next_scanline, nNV12Width, fStrides);
		...
		else
		{
			//cinfo.next_scanline += cinfo.image_height - cinfo.next_scanline;
			int nRestRow = cinfo.image_height - cinfo.next_scanline;
			unsigned char * pTempVUPlanar = (unsigned char*)malloc(8 * nNV12Width);
			if (pTempVUPlanar)
			{
				unsigned char * pUVLine = pTempVUPlanar;
				for (int i = 0; i < 4; i++)
				...
				//deitnerleave u and v
				deinterleave(pTempVUPlanar, uRows, vRows, 0, nNV12Width, fStrides);
